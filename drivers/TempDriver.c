...
Java_com_pi_devices_asynchronousdevices_TemperatureSensor_readSensor(JNIEnv *env, jobject obj, jint pin, jint type)
{
	...
	jclass sesnorReadingClass = (*env)->FindClass(env, "com/pi/devices/asynchronousdevices/TemperatureSensor$SensorReading");
	
	if(sesnorReadingClass == NULL)
		printf("jclass error");
	
	jmethodID constructor = (*env)->GetMethodID(env, sesnorReadingClass, "<init>", "(Lcom/pi/devices/asynchronousdevices/TemperatureSensor;FF)V");
